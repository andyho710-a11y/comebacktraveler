...
**Article list is a single source of truth:** `src/data/articles.js` exports `esimArticles`, `travelArticles`, `extraArticles`, and the combined `allArticles`. The homepage (`index.astro`) renders `esimArticles` + `travelArticles`; the RSS feed (`src/pages/rss.xml.js`, served at `/rss.xml`) uses `allArticles`. **When adding a new article: create its `.astro` page AND add an entry to the right array in `src/data/articles.js` with a `pubDate` (format `YYYY-MM-DD`)** — otherwise it won't appear on the homepage or in the RSS feed. Put articles that are live but not yet featured on the homepage in `extraArticles`. RSS sorts by `pubDate` newest-first.
